...
class Agent(object):
    ...
    def Human(self, othersID):
        """
        Decision-making policy for the Human agent:

        - If the opponent is Cooperator, Random, or Cheater, return "cheat".
        - If the opponent is Human, Copycat, or Grudger, return "cooperation".
        - If the opponent is Detective, return "cheat" for the first 4 interactions,
          then return "cooperation".
        - If the opponent is AI-like (AI/Q/DQN), return a random action.
        """
        var_opponent = None
        ...
        if var_opponent in ["Cooperator", "Random", "Cheater"]:
            ...
            return "cheat" if var_count < 4 else "cooperation"
        elif var_opponent in ["AI", "Q", "DQN"]:
            return self.RandomAction()
        else:
            return self.RandomAction()
    ...
    def get_state(self, opponent_id, opponent_stereotype):
        """
        Retrieve the most recent 5 complete interaction records with a specified opponent
        as the observation, and include the opponent's stereotype.

        Observation format:
        - The first part is the encoded stereotype of the opponent, represented as a string.
        - Then an underscore '_' is used as a separator.
        - Next are the most recent 5 interaction records. Each interaction is represented by two characters:
            - 'y' represents cooperation
            - 'n' represents cheating
            - 'oo' represents not yet completed
        - The final output is a string, for example: '3_ooooooyynn'.

        Parameters:
            opponent_id (int): The ID of the opponent.
            opponent_stereotype (str): The stereotype string of the opponent.
        ...
        # Iterate through the most recent 5 interaction records
        ...
        # If the number of interactions is fewer than 5, pad the beginning with 'oo'
