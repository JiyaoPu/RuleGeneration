...
document.addEventListener("DOMContentLoaded", function () {
  ...
  myLayout.registerComponent("StrategySetting", function (container, state) {
    ...
    strategyButton.once("change", function (val) {
      ...
      if (!runUrl) {
        console.warn("STATIC_ONLY enabled: skip /run_program");
        return;
      }
      fetch(runUrl, {
        method: "POST",
  ...
  myLayout.registerComponent(
    ...
    function (container, state) {
      ...
      var cooperationRateToggleDiv = container
        .getElement()
        .find("#cooperationRateToggle")[0];
      var cooperationRateToggle = new Nexus.Toggle(cooperationRateToggleDiv, {
        size: [40, 20],
        state: false,
      });
      ...
      var individualIncomeToggleDiv = container
        .getElement()
        .find("#individualIncomeToggle")[0];
      var individualIncomeToggle = new Nexus.Toggle(individualIncomeToggleDiv, {
        size: [40, 20],
        state: false,
      });
      ...
      var giniCoefficientToggleDiv = container
        .getElement()
        .find("#giniCoefficientToggle")[0];
      var giniCoefficientToggle = new Nexus.Toggle(giniCoefficientToggleDiv, {
        size: [40, 20],
        state: false,
      });
    }
  ...
  myLayout.registerComponent("ruleVisualization", function (container, state) {
    ...
    function updateChartsFromExcel() {
      const url = apiUrl("/rule_data");
      if (!url) {
        console.warn("STATIC_ONLY enabled: skip /get_excel_data");
        return;
      }
      fetch(url)
        ...
        .then((excelData) => {
          ...
          if (excelData.length > 0) {
            ...
            excelData.forEach(function (record) {
              ...
              window.lineChartCombinedData[1].y.push(
                record.reproduction_number
              );
              window.lineChartCombinedData[2].x.push(epoch);
              window.lineChartCombinedData[2].y.push(
                record.mistake_possibility
              );
            });
            Plotly.react(
              lineChartCombinedDiv,
    ...
    // 启动轮询
    const timerId = setInterval(updateChartsFromExcel, 5000);
    ...
    // // 10) SSE 监听 Excel更新通知
    // const sseUrl = apiUrl("/stream");
    // if (!sseUrl) {
    //   console.warn("STATIC_ONLY enabled: skip SSE /stream");
    //   return;
    // }
    ...
    function (container, state) {
      ...
      function updateAIImage() {
        ...
        if (img) {
          ...
          var newSrc = imgBase + "?" + new Date().getTime();
          img.src = newSrc;
  ...
  `;
      ...
      Plotly.newPlot(cooperationRatePieDiv, pieData, {
      ...
      Plotly.newPlot(individualIncomeHistogramDiv, incomeBarData, {
      ...
      function updateChartsFromExcel() {
        const url = apiUrl("/test_results");
        if (!url) {
          console.warn("STATIC_ONLY enabled: skip /get_test_results");
          return;
        }
        fetch(url)
          .then((response) => response.json())
          .then((excelData) => {
            ...
            if (excelData.length > 0) {
              ...
              var latest_coop =
                typeof latest.cooperation_rate === "string"
                  ? JSON.parse(latest.cooperation_rate)
                  : latest.cooperation_rate;
              var latest_income =
                typeof latest.individual_income === "string"
                  ? JSON.parse(latest.individual_income)
                  : latest.individual_income;
              var latest_gini =
                typeof latest.gini_coefficient === "string"
                  ? JSON.parse(latest.gini_coefficient)
              ...
              excelData.forEach(function (record) {
                var ep = record.epoch;
                var coop_vals =
                  typeof record.cooperation_rate === "string"
                    ? JSON.parse(record.cooperation_rate)
                    : record.cooperation_rate;
                var income_vals =
                  typeof record.individual_income === "string"
                    ? JSON.parse(record.individual_income)
                    : record.individual_income;
                var gini_vals =
                  typeof record.gini_coefficient === "string"
                    ? JSON.parse(record.gini_coefficient)
                    : record.gini_coefficient;
                for (var i = 0; i < categories.length; i++) {
          ...
          .catch((error) =>
            console.error("Error loading test results Excel data:", error)
          );
      }
      ...
      //   var data = JSON.parse(event.data);
      //   console.log(
      //     "Received SSE update in evaluationVisualization:",
      //     data
      //   );
      //   if (data.done) {
      //     console.log("Experiment finished");
      ...
      // eventSource.onerror = function (error) {
      //   console.error(
      //     "EventSource error in evaluationVisualization:",
      //     error
      //   );
      // };
    }
